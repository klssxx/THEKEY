...
| --- | --- | --- | --- |
| 1 | 0:00–0:18 | Clean title card, then `README.en.md` | The governance problem: plan, authority, physical execution, and release evidence. |
| 2 | 0:18–0:40 | README authorization flow | Review receipt → scoped human authority → policy → physical guard → one declared handler → gates → evidence. |
...
| 6 | 2:30–2:48 | README limitations, then closing title | Workflow isolation is not an OS sandbox; local grant is not a cryptographic signature; scan and action set are bounded. End on the value proposition. |
